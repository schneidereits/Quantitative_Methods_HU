...
=======
abies.df <- as.data.frame(abies_sample) 
>>>>>>> 6e3aee45ec4f3bc2469d639fb304d0806a525478
head(abies.df)
...
<<<<<<< HEAD
ggplot(abies.df, aes(x = Abies)) +
         geom_density()

m1 <- glm(data = abies.df, Abies ~ AnnualMeanTemperature + AnnualPrecipitation, family = "poisson")
=======
...
<<<<<<< HEAD
m_saturated <- glm(data = abies.df, Abies ~ AnnualMeanTemperature + Isothermality + TemperatureSeasonality + TemperatureAnnualRange + AnnualPrecipitation + PrecipitationSeasonality, family = "poisson")
=======
...
valid <- apply(abies.df, MARGIN = 1, FUN = function(x){any(is.na(x)) == FALSE})
abies.df[valid, "predicted"] <- predict(top_model, type = "response")

left_join(clim, abies.df)


```

abies.df$predicted <- predict(top_model, type = "response")

model_raster <- raster(clim)
model_raster

values(model_raster) <- bioclim_values$cluster
plot(kmean_raster)
